...
------------------------------------------------------------------------------------------------------------------------------------
int solution(std::string t, std::vector<int> c)
{
    int hour = stoi(t.substr(0, 2));
    int minute = stoi(t.substr(3, 2));
    int time = hour * 60 + minute;
    const int newyear = 24 * 60;
    int count = 0;
    if(time==0)  time = newyear;
    int rootTime = time;
    for(int i =0; i<c.size();i++){
        int nextTime = rootTime + c[i] - 60*i;
        time -= (i==0?0:60);
        if(time <= newyear && nextTime >=newyear) count++;
        time = nextTime;
    }
       time -=60;
