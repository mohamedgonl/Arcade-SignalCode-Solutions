...
The last printed page number.
  -------------------------------------------------------------------------------
  int numberDegree(int n){
    if(n<10) return 1;
    else return 1+numberDegree(n/10);
}
int solution(int current, int numberOfDigits) {
    if(numberOfDigits<numberDegree(current)) return current-1;
    else return solution(current+1, numberOfDigits-numberDegree(current));
}
